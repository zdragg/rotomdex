...
/// Wraps `embedded_io::Error`.
#[derive(Debug)]
pub struct EmbeddedIoError {
    underlying: Box<dyn embedded_io::Error + Send + Sync + 'static>,
}
...
impl error::Error for EmbeddedIoError {
    #[cold]
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        Some(&*self.underlying as _)
    }
...
impl DecodingError {
    ...
    #[cold]
    pub(crate) fn io(err: impl embedded_io::Error + Send + Sync + 'static) -> Self {
        Self::Io(EmbeddedIoError {
...
impl From<EmbeddedIoError> for DecodingError {
    #[inline]
    fn from(err: EmbeddedIoError) -> Self {
        Self::Io(err)
    }
}
