...
pub use self::decoder::{
    Decoded, DecodingError, DecodingFormatError, EmbeddedIoError, FrameDataType, FrameDecoder,
    OutputBuffer, PLTE_CHANNELS, StreamingDecoder, Version,
};
...
impl DecodeOptions {
    ...
    pub fn read_info<R: BufRead<Error: Send + Sync + 'static>>(
        self,
        r: R,
...
struct ReadDecoder<R: BufRead<Error: Send + Sync + 'static>> {
    reader: R,
    decoder: StreamingDecoder,
    at_eof: bool,
}

impl<R: BufRead<Error: Send + Sync + 'static>> ReadDecoder<R> {
    #[inline(never)]
    fn decode_next(
        &mut self,
...
pub struct Decoder<R: BufRead<Error: Send + Sync + 'static>> {
...
impl<R> Decoder<R>
where
    R: BufRead<Error: Sync + Send + 'static>,
{
    ...
    /// Abort decoding and recover the `embedded_io::BufRead` instance
...
pub struct DecoderIter<R: BufRead<Error: Send + Sync + 'static>> {
    inner: Decoder<R>,
    ended: bool,
}

impl<R: BufRead<Error: Send + Sync + 'static>> DecoderIter<R> {
    /// Abort decoding and recover the `embedded_io::BufRead` instance
    ///
