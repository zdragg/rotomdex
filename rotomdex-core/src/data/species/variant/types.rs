...
impl ModelTypes {
    ...
    pub(crate) fn def_effectiveness(&self) -> ModelTypeEffectiveness {
        let Some(secd) = self.secondary else {
            ...
            return ModelTypeEffectiveness::new(
                &[],
                mono.def_2x(),
                mono.def_halfx(),
                &[],
        ...
        prim.def_2x()
            .iter()
            .for_each(|t| counter[*t as usize] <<= 1);
        secd.def_2x()
            .iter()
            .for_each(|t| counter[*t as usize] <<= 1);
...
#[derive(
    Clone, Copy, Debug, EnumString, Display, PartialEq, PartialOrd, Eq, Ord, EnumCount, EnumIter,
)]
