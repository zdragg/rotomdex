...
impl ModelEvolutionDetail {
    pub(crate) fn to_strings(&self) -> Vec<String> {
        ...
        for (label, name) in [
            (
                "use item:",
                detail.item.as_ref().map(|item| item.name.as_str()),
            ),
            (
                "hold item:",
                detail.held_item.as_ref().map(|item| item.name.as_str()),
            ),
            (
                "knows move:",
            ...
            (
                "use move:",
                detail.used_move.as_ref().map(|move_| move_.name.as_str()),
            ),
            (
                "location:",
                detail
            ...
            (
                "region:",
                detail.region.as_ref().map(|region| region.name.as_str()),
            ),
            (
                "species in party:",
                detail
    ...
    fn new(detail: Vec<EvolutionDetail>, settings: Settings) -> ResourceResult<Self> {
        let detail = detail
            ...
            .filter_map(|detail| {
                ...
                if version_group > settings.version.version_group() {
                    return None;
                }
